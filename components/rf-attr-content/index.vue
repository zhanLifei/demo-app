			...
			<view class="right">
				...
				<view class="sku-info-wrapper">
					<view class="price-wrapper">
						...
						<text :class="'text-' + $mStore.getters.themeColor.name" v-if="price">{{ moneySymbol }}{{ currentProductPrice }}</text>
					</view>
				...
				<view class="item-list">
					...
					>
						<view
							...
							:class="[childItem.selected ? 'bg-' + $mStore.getters.themeColor.name : 'tit-normal', childItem.disabled ? 'disabled' : '']"
							:style="
...
<!--					:max="parseInt(stock || product.stock, 10)"-->
		...
		<button v-if="!showBuyBtn" class="btn" :class="'bg-' + $mStore.getters.themeColor.name" @tap="toggleSpec">完成</button>
		<view class="btn-group" v-else>
			<button class="btn" :class="'bg-' + $mStore.getters.themeColor.name" @tap="toggleSpec(1)">加入购物车</button>
			<button class="btn" :class="'bg-' + $mStore.getters.themeColor.name" @tap="toggleSpec(2)">立即购买</button>
		</view>
	</view>
</template>
<script>
